      ...
      <div className="w-full h-full flex flex-row items-center justify-end gap-4">
        <AnimatePresence mode='wait'>
          {loggedIn && <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{type: "spring", stiffness: "500", damping: "30"}} exit={{ scale: 0, opacity: 0, transition: {duration: 0.1} }} key={"user"} className='flex flex-row items-center gap-2'>
            ...
            <motion.div ref={notificationsRef} onClick={(e) => {e.stopPropagation(); e.preventDefault()}} style={{originX: 0.925, originY: 0}} animate={showNotifications ? 'visible' : 'hidden'} variants={{visible: {opacity: 1, scale: 1}, hidden: {opacity: 0, scale: 0}}} className={`absolute w-72 h-80 right-24 top-[5.5rem] bg-neutral-800 cursor-default rounded-lg flex flex-col items-start justify-start ${showNotifications ? 'pointer-events-auto' : 'pointer-events-none'}`}>
